...
teamData <- teamData %>% filter(season == 2009, between(week, 4, 9))
...
schedule <- schedule %>% filter(season == 2009, between(week, 4, 9))
...
logModel <- train(data = trainData, homeWin ~ offRushYdsDiff + offPassYdsDiff, method = "glm", family = "binomial")
